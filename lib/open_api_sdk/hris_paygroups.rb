  ...
  class HrisPaygroups
    ...
    def retrieve(x_connection_token, id, remote_data = nil)
      ...
      url = Utils.generate_url(
        ::OpenApiSDK::Operations::RetrieveHrisPaygroupRequest,
        base_url,
        '/hris/paygroups/{id}',
        request
      )
      headers = Utils.get_headers(request)
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::RetrieveHrisPaygroupRequest, request)
      headers['Accept'] = 'application/json'
      headers['user-agent'] = @sdk_configuration.user_agent
      ...
      res = ::OpenApiSDK::Operations::RetrieveHrisPaygroupResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Shared::UnifiedHrisPaygroupOutput)
          res.unified_hris_paygroup_output = out
        end
      end
      res
    end
